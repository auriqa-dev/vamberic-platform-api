...
No raw JSON/document, Mongo `_id`, internal DB metadata, request headers, authentication tokens or IP address is copied into the email. The explicit context projection is the boundary; user-written message content itself is still included. Event/Product IDs are used for safe log correlation. Logs do not contain recipient/sender addresses, names, subjects, bodies, credentials, raw provider errors or SDK error causes. AWS request logging is not enabled.
